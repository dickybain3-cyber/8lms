...
import { csvToGuruRows } from "@/lib/csv";
import {
  importGuruBatch,
...
const CONTOH_CSV = `nama,email
Budi Santoso,budi.santoso@sekolah.sch.id
Rina Wulandari,rina.wulandari@sekolah.sch.id`;
...
export default function ImportGuruForm() {
  ...
  function handleParse() {
    ...
    const rows = csvToGuruRows(csvText);
    if (rows.length === 0) {
      setParseError(
        "Tidak ada baris data terbaca. Pastikan baris pertama adalah header 'nama,email'."
      );
      return;
    }
    const withNomor: ImportGuruInputRow[] = rows.map((r, idx) => ({
      nomorBaris: idx + 1,
      ...r,
    }));
  ...
  function downloadHasil() {
    ...
    const body = berhasil
      .map((r) => {
        ...
        return `"${r.nama}","${r.email}","${r.password}"`;
      })
      .join("\n");
        ...
        <>
          <div className="rounded-lg border border-ink/10 bg-white p-4 sm:p-5">
            <p className="mb-3 text-sm text-ink/70">
              Kolom wajib: <code className="text-xs">nama</code>,{" "}
              <code className="text-xs">email</code>. Guru login pakai
              email asli ini (bukan username seperti siswa) — pastikan
              setiap email belum pernah dipakai untuk akun lain di sistem
              ini.
            </p>
          ...
          {status === "parsed" && (
            <div className="rounded-lg border border-gold/30 bg-gold/5 p-4 text-sm text-ink/70">
              ...
              Baris dengan email yang formatnya tidak valid, atau sudah
              terdaftar di sistem, akan dilewati otomatis saat import
              (dicatat di ringkasan).
            </div>
          )}
